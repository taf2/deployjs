/*
 * deployjs
 * Copyright(c) 2011 taf2 <todd dot fisher at gmail>
 * MIT Licensed

deployjs provides an easy way to deploy node.js applications using the cluster.js server

To start you need a .deployspec file in your project root and a deploy.js control file

.deployspec file is JSON and contains information about where your app will be deployed
  {
    "name": "your_app_name", // the name of your application
    "dir": "/var/www/apps/", // root folder for your application installations
    "host": "server", // the name of the server you'll be targeting for deploy
    "branch": "gitbranch" // the gitbranch you're deploying from
  }

deploy.js: simple file to control executing the deploy functions
  require('deployjs').CLI();

To control the deploy type the following:
...
var Commands = (function() {
  ...
  if (process.env.NODE_ENV == 'test') {
    // allow our internal definitions to be replaced for testing
    module.exports.stub = function(name, func) {
      console.log("stub: %s", name);
      eval(name + "=" + func.toString());
    }
